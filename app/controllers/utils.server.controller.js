...
var sendgrid           = require('sendgrid')('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'),
...
exports.sendDeclinedProjectEmail = function (res, to, variables, toCompanyAdmin) {
    res.render(
        ...
        function (err, html) {
            if (err) {
                ...
                sendgrid.send({
                    to     : to,
                    subject: 'Scanifly :: Project Declined Notification!',
                    from   : config.mailer.from,
                    html   : html
                }, function (err, info) {
                    if (err) {
    ...
    res.render(
        ...
        function (err, html) {
            if (err) {
                ...
                sendgrid.send({
                    to     : to,
                    subject: 'Scanifly :: Flight Rescheduled Notification!',
                    from   : config.mailer.from,
                    html   : html
                }, function (err, info) {
                    if (err) {
...
exports.getNotifiedUsersPerProject = function (req, projectId, callback) {
    ...
    projectsController.getProjectByID(projectId, function (project, err) {
        ...
        var projectUrl = req.protocol + '://' + req.hostname + '/#!/project-upload/' + project._id;
